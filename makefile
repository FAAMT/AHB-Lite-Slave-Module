...
COMPONENT_FILES	:= ahb_lite_slave.sv fir_filter.sv coefficient_loader.sv controller.sv counter.sv flex_counter.sv sync_low.sv magnitude.sv 

# Specify the name of the top level file (do not include the source folder in the name)
# NOTE: YOU WILL NEED TO SET THIS VARIABLE'S VALUE WHEN WORKING WITH HEIRARCHICAL DESIGNS
# AND THE AUTOMATED GRADING SYSTEM
TOP_LEVEL_FILE	:= ahb_lite_fir_filter
...
LIB_CREATE	:= vlib
COMPILE 		:= vlog -sv
SIMULATE		:= vsim -Lf $(LABS_IP_LIB) -L $(GATE_LIB) -L $(GOLD_LIB) +no_glitch_msg -coverage -voptargs="+acc" -suppress 12110 -do ahb_lite_fir_filter.do #ahb_lite_slave.do
DC_SHELL		:= dc_shell-t
